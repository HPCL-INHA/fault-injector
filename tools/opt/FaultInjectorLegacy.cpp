...
//===----------------------------------------------------------------------===//
//
//  Copyright (C) 2019-2020. rollrat. All Rights Reserved.
//
//===----------------------------------------------------------------------===//
...
class FaultInjectionInsertMachine {
 public:
  ...
  static void insertMetaFunc(Module &M) {
    ...
    //
    FunctionCallee initFunc = getInitFunction(M);
    CallInst::Create(initFunc, "", entryBB->getFirstNonPHI());

    //
    // Insert 'fault_inject_finish' function.
    //
    FunctionCallee finishFunc = getFinishFunction(M);
  ...
  static bool insertFaultInjection(Module &M, Function &F, Instruction *I,
    ...
    FunctionCallee inject_func = getInjectFunc(M);
    FunctionCallee trace_func = getTraceFunc(M);
    ...
    FunctionCallee injectfunc = M.getOrInsertFunction(funcname, injectfunctype);
    ...
    for (std::list<User *>::iterator use_it = inst_uses.begin();
      ...
      if (isa<Instruction>(user) && dyn_cast<Instruction>(user)->isTerminator()) continue;
...
#pragma region Meta Functions
  ...
  static FunctionCallee getInitFunction(Module &M) {
  ...
  static FunctionCallee getFinishFunction(Module &M) {
    LLVMContext &context = M.getContext();
...
#pragma region Fault Injection Functions
  ...
  static FunctionCallee getTraceFunc(Module &M) {
  ...
  static FunctionCallee getInjectFunc(Module &M) {
    std::vector<Type *> profile_func_param_types(7);
    LLVMContext &context = M.getContext();
    profile_func_param_types[0] = Type::getInt32Ty(context);  // function index
    profile_func_param_types[1] = Type::getInt32Ty(context);  // index
  ...
  static FunctionCallee getDetermineFunc(Module &M) {
  ...
  static void createInjectionFunctions(Module &M) {
    FunctionCallee pre_fi_func = getDetermineFunc(M);
    FunctionCallee injectfunc = getInjectFunc(M);
    FunctionCallee tracefunc = getTraceFunc(M);
...
static RegisterPass<LLVMFaultInjectionPass> X("faultinjectlegacy",
                                              "Fault Injection Legacy Pass",
